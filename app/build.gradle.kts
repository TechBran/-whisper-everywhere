...
android {
    ...
    defaultConfig {
        ...
        targetSdk = 36
        versionCode = 93
        versionName = "4.5.1"  // THE LIVE WORDS ARE RED, AND THEY ARRIVE ON THE FIRST TAP. Two owner reports from the 4.5.0 device session. First, the friction: a freshly installed language armed only on the SECOND dictation, which a user reads as "it does not work" - and the cause was that the previewer was warmed at only two moments, service start and session start, so an install landing in between waited for the next session (a mid-session install cost TWO). The set of moments is now ENUMERATED - service start, selection changed, pack installed, session start, session end, memory trim - one decision answers all six, and a seventh added later fails to compile rather than silently missing. Re-picking a language whose pack was already installed, and merely leaving the app (a UI-hidden trim frees the recognizer), were two more instances of the same miss and are closed too. Second, the colours: the live strip and the committed transcript are now separately settable, with nineteen swatches and the live default a legible red - the strip is shared by the local previewer and all four live cloud providers, so one setting colours every live moment. The bubble background gains an opacity control with an 85% floor, which is the lowest step at which every swatch still clears 4.5:1 over a WHITE backdrop - below it the bubble stops being able to promise readability at all, because the thing behind the text becomes someone else's app. 4.5.0 was the six languages (92, on the internal track).
