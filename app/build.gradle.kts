...
android {
    ...
    defaultConfig {
        ...
        targetSdk = 36
        versionCode = 94
        versionName = "4.5.2"  // ALL SEVEN LANGUAGES CLEARED FOR PRODUCTION, AND THE NOTICES THE LICENCES ASK FOR IN RETURN. The owner did the licensing research himself and on 2026-09-13 accepted the reviewed basis for every preview pack, so German, Russian, Indonesian, Korean and Chinese-English join English and French: the record names him as the decision owner, states each row's real basis, and marks every remaining uncertainty ACCEPTED rather than resolved. Two things it had asserted as fact are corrected - the Chinese corpus is UNDISCLOSED (the card says mix, sherpa's docs say an internal corpus, and the 12k-hour figure belongs to a fork parent, so the non-commercial claim that made it the weakest row is withdrawn), and Korean rests on AI-Hub's published FAQ, which permits selling models trained on its data with attribution, rather than on the access clauses that govern who may download the data we never possess. And the other half of the trade now ships: the full Apache-2.0 and MIT texts, per-pack attribution at pinned revisions, the corpora that ask for credit - Common Voice, LibriSpeech, and KsponSpeech with AI-Hub as its source, which is the CONDITION Korean is cleared on - and a plain statement of our own modifications, in the base app where the pack pipeline cannot delete them and reachable from a screen. A licence text that ships in a folder nobody opens is not delivered. 4.5.1 was the first-tap fix and the bubble colours (93, on the internal track).
