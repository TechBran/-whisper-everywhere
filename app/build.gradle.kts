...
android {
    ...
    defaultConfig {
        ...
        targetSdk = 36
        versionCode = 95
        versionName = "4.6.0"  // THE INSTRUMENT LADDER, AND THE CHOOSER GOES MULTILINGUAL-ONLY. The owner ruled that English-only tiers stop being offered, so small.en is retired the gentle way (hidden from the chooser, its installed users left completely alone) and the default moves to the multilingual small model - the only rung with a MEASURED throughput pass. Then five rungs join it and large-v3-turbo comes back, every one an INSTRUMENT: selectable on any device, never recommended, never the default, never a migration target. They exist so the owner can decide the CPU tier on six real devices instead of from a prediction, and two of them - small and medium at Q8_0 - test the finding that matters most: the app ships only Q5_0/Q5_1, the two quantisations absent from ggml's ARM i8mm repack path, so its finalizer may be running at a fraction of what the hardware delivers. The chooser is honest in both directions - no CPU rung claims speed, the heavy rungs say they may not keep up, and the NPU cards keep the "fastest" claim that is true of them. PROMOTION IS GATED: a rung with no recorded throughput verdict cannot go to production, in the same shape as the language clearance gate, so a habit promotion cannot ship a model that falls silently behind the strip. 4.5.2 was the seven-language clearance and the notices (94, in production).
