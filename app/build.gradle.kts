...
android {
    ...
    defaultConfig {
        ...
        targetSdk = 36
        versionCode = 91
        versionName = "4.4.1"  // THE PACK FOLLOWS THE LANGUAGE YOU PICK. 4.4.0 shipped the word-for-word previewer behind a setting nobody had to find, so nobody found it: the pack now arrives on its own. Picking a language - at onboarding or later - fetches that language's preview model on the spot, and a foreground top-up catches the user who picked English before this release and would never re-pick it; both are ONE collector on the selected-language flow, so fetching, loading and arming all read the same single input. Silent on validated unmetered wifi, a sized one-tap card on anything metered, and a "no" - a dismiss or a delete - is remembered PER LANGUAGE and never undone. Auto gets no live words at all and now says so plainly on all four surfaces (owner ruling); the English pin that whisper applies to its own English-only tiers no longer leaks into the previewer, so that promise is literal rather than nearly true. A language with no preview model is told so instead of being sold the English one. And the recognizer is no longer loaded - 169 MB and an 800 ms load - for a session that can never arm it. 4.4.0 was the previewer itself, the four-pack bundle and the startup ring (90, on the internal track).
