...
android {
    ...
    defaultConfig {
        ...
        targetSdk = 36
        versionCode = 92
        versionName = "4.5.0"  // SIX MORE LANGUAGES FOR LIVE WORDS. The word-for-word previewer stops being English-only: French, German, Russian, Indonesian, Korean and a Chinese-English bilingual join it as on-demand packs, 421,369,659 B across six modules, and the pack for the language you pick arrives on its own. Almost all of the work was the SEAM, not the languages - warm() is idempotent on the PACK rather than the engine, modelType goes out EMPTY so the encoder's own metadata decides the family (French is a v1 export and a mismatch is an uncatchable process kill), the audio pad is DERIVED from each pack's own T (Russian and Indonesian are T=77, where the shipped 500 ms would have silently eaten the last word of every utterance), the canary and its disabled latch are per-pack, and the strip is built from TOKENS so Korean keeps its word spaces. One observable per language now answers "what is this feature doing" for both screens, replacing a two-variable answer that three review rounds proved wrong. Download policy is the owner's: picking a language fetches its pack silently, on any connection, with no tap - the pick IS the consent - and the progress sits above the language selector where the choice was made. PUBLICATION IS GATED: en and fr are cleared for production; de, ru, id, ko and zh ride the internal track with their corpus clearance outstanding and a test that fails if the switch outruns the evidence. 4.4.1 was the acquisition model on one pack (91, on the internal track).
