...
android {
    ...
    defaultConfig {
        ...
        targetSdk = 36
        versionCode = 90
        versionName = "4.4.0"  // THE WORD-FOR-WORD PREVIEWER. A streaming Zipformer (73 MB, English) mirrors the microphone beside whisper and puts words on the bubble strip ~0.4-0.5 s behind the voice; the VAD keeps cutting utterances for whisper (turbo on the NPU, small on CPU) and whisper's cased, punctuated, multilingual final replaces the preview and is what gets typed. Both model payloads move onto Play Asset Delivery - preview_en (73 MB) and tts_kokoro (350 MB) beside the two NPU packs, four packs in one bundle - so an upstream re-upload can never break an install again (it did, 2026-09-08 to 09-10). And the startup ring: the recorder opens at the TAP, the audio between the tap and a ready engine is buffered and replayed, the prewarm re-arms after a memory trim, and the "listening" cue moved onto real readiness - the cold NPU load is 4,107 ms measured, which is what used to eat the first couple of seconds. 4.3.4 was Gemini Live (89, on the internal track; 88 before it).
