...
android {
    ...
    defaultConfig {
        ...
        targetSdk = 36
        versionCode = 89
        versionName = "4.3.4"  // 89 supersedes 88 on the internal track (88 = the owner-confirmed first Gemini Live build): Gemini Live is now ON BY DEFAULT once a key is entered (owner ruling 2026-09-10), the turbo card says what it is (fastest and most accurate on this device), and the voice-archive integrity gate accepts the known-good set after k2-fsa re-uploaded the Kokoro archive on 2026-09-08 and broke every fresh voice install for two days. Gemini Live transcription as the fourth live provider: manual voice activity driven by the app's own endpointer (the API's automatic detection goes deaf after the first sentence), binary WebSocket frames, close-frame classification with reconnect (a bad key used to end the cloud half silently), GoAway rotation at 540 s; plus the cloud-language leak fix from the 3.8 spec and the sherpa-onnx runtime bump to 1.13.7. 4.3.3 was the accessibility service becoming RECOMMENDED, not required: onboarding and the home gate need mic + overlay only; without the service a dictation ends with the transcript on the clipboard and a toast; the card says what THIS device can do (a Galaxy XR forbids third-party accessibility services by policy; Android 13+ Restricted Settings blocks non-store installs — suspected only when the install source says so). Read from the owner's headset 2026-09-04. 4.3.2 was the silence fix: speech evidence gates the encode (the NPU path never had the CPU tier's pre-encode Silero filter, so a 15 s bed of room tone reached the decoder and came back as "Thank you for watching" or a Chinese subtitle credit) plus the stock-phrase blocklist under an elevated no-speech vote. 4.3.1 shipped as versionCode 84 (the flatline cut) with 85 on the internal track (backpressure governor, detect-margin line, honest Auto copy). 4.3.1 was the field-report release. Five workstreams, all from the owner using 4.3.0: whisper.cpp decode guards on the NPU (no-speech at SOT, per-token log-prob, in-loop entropy with a cycle signature, the temperature ladder) so turbo stops repeating a word 70-80x and stops typing "Thank you" from dead time; the bubble survives a read (one hide sink, a deferred replay); projected-complete TTS playback so there is no buffering; the screen-capture consent asks at most twice per session; and three fixes found on the phone after the merge - the ribbon animates per SECOND not per frame, the microphone never enters a device-audio session, and a paused video is no longer read as a DRM block. Carries the 4.4 VAD hangover retune too (HANGOVER_MS 500 -> 350; npu-turbo cadence floor 2,000 ms by owner ruling over the 0.70 duty rule, one sentence per chunk) and, at versionCode 84, THE FLATLINE CUT: on device-audio sessions a chunk run at RMS <= 10 for 5 chunks cuts like a hangover close, because edited YouTube leaves 100-300 ms of digital silence where the sentence boundaries were and Silero alone never closes there. 83 was uploaded to the internal track 2026-09-03 and superseded there; 84 was PROMOTED TO PRODUCTION 2026-09-03 (E8 passed: median interval 12.0 s -> 3.94 s on edited video). 85 adds the BACKPRESSURE GOVERNOR (turbo floor 2,000 while the segment queue is <= 1 deep, 3,200 once it reaches 2, back at <= 1 — the adaptive guard the 2,000 ruling was a placeholder for), the always-on margin on the native detect: line (the measurement for the silence-hallucination fix), and honest Auto onboarding copy. (4.3.0/82 was one-tier-per-device; 4.2.0/81 Fleet Onboarding.)
