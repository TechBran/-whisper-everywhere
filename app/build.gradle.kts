...
android {
    ...
    defaultConfig {
        ...
        targetSdk = 36
        versionCode = 97
        versionName = "4.8.0"  // FOUR RULINGS FROM THE 96 TABLET SESSION (owner, 2026-09-17, after seeing 4.7.0/96 on his Tab S10+: "Everything looks good"). (1) The transcript window's scrollbars are VISIBLE - persistent, 4dp, a drawn thumb on a faint track on both the committed text and the live strip; the framework's fade-at-rest default is why earlier builds "don't seem to have that". (2) "Keep bubble always on screen" defaults to OFF for NEW installs; an existing install that never touched the toggle has `true` written once at construction so nobody's bubble changes mode on upgrade. (3) The opacity ladder goes down to 20% so a video plays through the panel; the legibility guarantee is kept honest at 85% and above (OPACITY_GUARANTEED_PERCENT) and the slider states the trade below it. (4) First-run model choice: NPU-capable devices see turbo alone (already true); every other device is gated at the owner's 4.5 GB - under it the smallest Q8 rung, over it medium and turbo with medium steered. 96 was 4.7.0, sideloaded to the tablet only, never on a Play track; 97 is for the internal track. Production authorisation is still withheld pending the owner's accuracy pass (the promotable set is empty). Previous: THE Q8 LADDER. On 2026-09-17 five CPU rungs were timed on the owner's Tab S10+ against the same talk (docs/measurements/2026-09-17-tab-cpu-ladder.md) and the owner ruled the same day: "Q8 for everything" - "Q5 is definitely off the table". Small Q8_0 becomes the floor for every device and the default (the same weights as the 190 MB model, 2.2x faster per commit on that tablet), medium Q8_0 becomes the medium tier recommended above a provisional 5.5 GB RAM threshold, and large-v3-turbo Q8_0 stays an optional top rung - it kept up on a flagship with no margin, so it is offered for its accuracy and never advocated. The four Q5 rungs (the shipped 190 MB default among them) are retired the gentle way: hidden from the chooser, untouched for anyone who has one. PRODUCTION AUTHORISATION IS WITHHELD: the promotable set is empty pending the owner's accuracy pass on small and medium, so 96 goes to the internal track and a sideloaded tablet only. 95 was 4.6.0, the instrument ladder, never promoted.
