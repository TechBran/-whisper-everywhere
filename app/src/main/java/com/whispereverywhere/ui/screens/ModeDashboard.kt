 ...
 * raw shared preference, because Gemini's live mode (4.3.4) lives on its own flag: the shared flag
 * is deliberately NOT reset when the engine switches, so passing it raw would surface "real-time
 * streaming" over a Gemini session the user had switched to BATCH on Gemini's own row. The chip re-applies the realtime rule itself
