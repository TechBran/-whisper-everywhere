...
### 6.6 Batch transcription mode (transcribe an audio FILE, in the main UI)

**NEW 2026-07-29, RESCOPED same day by owner ruling.** The original section (recoverable from git
history at `f020d0a`) designed an in-app record surface with a saved-recordings library, 30-day
retention, and a provenance-gated engine path. The owner overruled it, verbatim: *"it just needs to
be a file picker that you can select an audio recording and push it into a transcription. That's
it. No need to save recordings. We can just transcribe exactly what we have as an audio file."*

So batch mode is now: **pick an audio file, transcribe it all at once through the chosen engine
(local model or selected cloud provider), get the transcript.** No record screen, no recordings
library, no retention policy. The transcript lands in the existing text-only `TranscriptStore`
history; the only audio the app holds is a transient decode workspace in cache, deleted when the
job completes.

**Decisions, revised (superseding D26-D34 where named):**

| # | Decision | Choice | Why |
|---|---|---|---|
| D26r | Inlet | **SAF file picker (`ACTION_OPEN_DOCUMENT`, `audio/*`), the ONLY inlet** | Owner ruling. No in-app recording, no bubble-teeing. |
| D27r | Saved recordings | **NONE.** Transient decode workspace in `cacheDir/batch/<uuid>/`, deleted on `Done`/user dismissal, stale-swept (>24 h) on app start | "No need to save recordings." Cache is non-backed-up and OS-clearable by definition -- the Auto-Backup leak par.6.2 worried about cannot occur. |
| D28r | Provenance / BatchRouting | **CUT -- dead code by construction.** Batch mode has NO capture path: it never touches the microphone or MediaProjection, so the device-audio promise (par.3.2, privacy par.6-7) is unreachable from here. The live path's `SourceRoutedTranscriptionEngine` remains the sole and sufficient enforcement point. | An imported file is the user's own content, explicitly pushed at an explicitly chosen engine -- a user-directed upload, not captured third-party audio. |
| D29r | Codec surface | **IN (was the reason import was cut; now unavoidable).** `MediaExtractor`+`MediaCodec` decode -> downmix -> resample to 16 kHz mono PCM16, streamed to the cache workspace. ONE uniform pipeline: cloud chunks are cut from the DECODED PCM and WAV-wrapped at dispatch, never sent as the original container. | Chunking a compressed file requires decoding anyway (a byte-offset cut mid-mp3-frame is garbage); one pipeline means the 25 MB math, silence scan, and re-slice logic all apply unchanged. |
| D30 | Retry semantics | **KEPT (narrowed):** per-chunk checkpoint in the workspace manifest; Retry on a Failed job resumes non-`Done` chunks without re-uploading `Done` ones. The workspace lives until `Done`, so retry needs no saved library -- it just needs the job not to have succeeded yet. | Re-uploading `Done` cloud chunks double-bills a paid API. |
| D31 | Job host | **KEPT:** foreground `dataSync` service + per-chunk checkpoint | An hour-long file decodes + transcribes for many minutes; backgrounded plain coroutines get killed. |
| D32 | Chunk fallback | **KEPT:** automatic per-chunk one-way cloud->local (with re-slice to the local ceiling); `Fatal` latches | Unchanged. |
| D33 | Silence detection | **KEPT as revised at planning:** pure-Kotlin `SilenceScanner` energy scan for the coarse cut; Silero still runs per-chunk inside `whisper_full` | See the planning deviation note -- the Silero seam is native-internal and whole-clip scanning is the OOM chunking avoids. |
| D34 | Ordering | **KEPT:** strictly sequential, `SegmentOrderer` skipped | Output lands on a screen, not an injected IME field. |

**The pipeline.**

1. **Pick.** Home card "Transcribe audio file" -> `ACTION_OPEN_DOCUMENT` (`audio/*`). Duration is
   read cheaply via `MediaMetadataRetriever` (no decode) for the pre-flight cost estimate.
2. **Consent + cost, per job.** The engine for THIS job is shown and choosable at the picker result
   ("On-device" / the selected provider) -- defaulting to the global selection. Pushing a file at a
   named cloud provider is explicit per-file consent; the global disclosure v2 stays as-is (its
   meaning is unchanged -- same triad, same provider). With a cloud engine and
   `BatchCostEstimator.needsConfirmation(durationMs)`, an explicit confirm dialog runs BEFORE the
   service starts ("about N min, about cX, with your OpenAI key"). The service re-checks on the
   DECODED byte length and degrades to local if unconfirmed -- no stale intent can start a large
   cloud job silently.
3. **Decode** (in the foreground service): `MediaExtractor`+`MediaCodec` -> `Downmix.toMono` ->
   `Resampler.to16k` (both pure Kotlin, unit-tested; the `MediaCodec` loop itself is framework code
   and is DEVICE-verified -- under `unitTests.isReturnDefaultValues = true` it is untestable in JVM
   tests, the par.3.4 trap) -> PCM16 streamed through `PcmSink` into `cacheDir/batch/<uuid>/audio.pcm`,
   gated by `StorageGuard` on an estimate from the retriever duration (decoded size =
   32,000 B/s x duration). Unsupported/corrupt file -> the job fails with an honest message before
   any engine work.
4. **Chunk + transcribe:** exactly the planned machinery -- `SilenceScanner` + `ChunkPlanner`
   (cloud ceiling 20 MB, local ~90 s), `BatchTranscriber` (sequential, checkpointed, resumable,
   per-chunk one-way fallback with re-slice, native access confined to its own single thread),
   `WavWriter.wrap` per cloud chunk, `gpt-transcribe` (par.5.6e).
5. **Deliver:** the assembled transcript is saved into the existing `TranscriptStore` (so it
   appears in the normal Transcriptions history) and shown with Copy / Share. The workspace --
   PCM and manifest -- is deleted on `Done`; on `Failed`/`PartiallyDone` it survives for Retry
   until the user dismisses the job or a stale sweep (>24 h) collects it.

**Compliance deltas (rescoped).**

- **Privacy policy par.6 needs ONE clause** (both copies + the mirrored sentence in the ToS): today
  it says a selected provider *"only ever receives audio you dictate yourself through the
  microphone."* With file import, that becomes: microphone dictation **or an audio file you
  explicitly choose to transcribe with that provider**. The MediaProjection carve-out is untouched
  and stays absolute -- batch has no capture path at all.
- **Data Safety: still NO flip.** The user-initiated audio upload to a user-keyed provider was
  declared in C2a; a picked file rides the identical, identically-gated transmission. Transient
  cache processing of a user-selected file is not new collection.
- **No stored-audio narrative needed** -- there is no recordings library to describe. The
  PLAY-DECLARATIONS par.5 "text-only" description of on-device storage stays TRUE (cache workspace
  is transient processing, not a store), which deletes the amendment the library design required.
- **No speed claims** anywhere (invariant #5, unchanged).

**Scope cuts (OUT -- this release).** In-app recording UI; saved-recordings library / retention /
deletion UI; audio playback or scrubbing; audio export / `FileProvider`; parallel chunk upload;
`SegmentOrderer` in batch; `SegmentQuality` wiring; cloud TTS; C4 streaming (`gpt-live-transcribe`,
par.5.6e); Gemini/ElevenLabs batch (C2b); translation; audio editing.

**Open questions: none.** The four from the original section were all answered by the owner ruling
of 2026-07-29 (import IS the feature; no playback; no library; no retention).
