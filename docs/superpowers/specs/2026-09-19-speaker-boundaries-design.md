...
2. ~~If the spike shows layer 2 costs more than ~1 s per chunk even gated, do we ship layer 1 alone or tighten the gate?~~ **RULED 2026-09-19: decide it on the device, not on the number** — *“I'm not sure if one second is much or not, but let's get to that point and test it and see if it feels okay in real world.”* So the spike reports the cost and ships the build regardless; the gate is then tuned against what he feels. **What he can actually feel** is bounded and worth stating so the judgement is about the right thing: nothing during a session (the work is on the speaker thread, below delivered text, and commit cadence is untouched), a longer wait at the stop tap (the finalize fence, 3,000 ms today, is where per-chunk cost turns into a pause the user sees), and battery and heat over a long session. Those three are the acceptance question, not the millisecond count.
