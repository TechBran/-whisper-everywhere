...
- **Per-provider fallback (documented, not built).** If a provider genuinely lacked server segmentation, THAT provider would keep client-VAD commits (deltas still streaming) via `serverDriven=false`. Precisely: the ENGINE's client-VAD ledger is retained and tested, but each protocol's client-commit machinery (OpenAI's commit frame, ElevenLabs' fold-slot/timeout, Soniox's grace window) is REMOVED by this wave, not retained — so re-enabling the fallback would mean rebuilding that protocol path, not just flipping the flag. All three verified providers segment server-side, so building an unused client path would be dead code; the `serverDriven` seam is kept as a construction flag so the fallback stays documented, not built.
