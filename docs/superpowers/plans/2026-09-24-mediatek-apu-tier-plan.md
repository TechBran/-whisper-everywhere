...
2. **The speed claim** on MediaTek families — the owner's wording (proposed: "the most accurate model this
   ...
   pin guarded it until he ruled. Its accuracy-only body is "The most accurate model that runs on
   ...
   *Ruled 2026-09-25: accuracy only.* The owner, at the end of the Tab S10+ ship session of 4.16.0/113: "the
   MediaTek speed claim copy is fine for now." The headline ("Best AI-chip accuracy") and the body stay exactly
   as they are, with no speed claim — on the tablet the CPU's small model commits in ~1.2 s against the APU's
   ~2.3 s, so the APU's win is accuracy — and the pin stays, renamed
   `the_qualcomm_speed_sentence_never_renders_on_a_mediatek_row_by_ruling`: it guards a ruling, not a pending
   decision. Done (design §2.8).
